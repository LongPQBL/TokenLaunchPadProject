...
function setup(over: Partial<SelfCustodyDeps> & { logs?: unknown[]; status?: "success" | "reverted"; allowance?: bigint; batchLogs?: unknown[] } = {}) {
  ...
  const deps = {
    ...
    account: variant.address,
    kind: variant.kind,
    signer: variant.signer,
    chainId: CHAIN,
...
// The whole suite runs twice, per spec §14: once for the person's own wallet and once for the session wallet, which is the
// same seam over a local account. Everything that must hold for one must hold for the other.
const SESSION_ACCOUNT = privateKeyToAccount(generatePrivateKey());
const VARIANTS = [
  { name: "self-custody signer", kind: "self-custody" as const, address: USER, signer: USER as Address | typeof SESSION_ACCOUNT, zeroPrompt: false },
  { name: "session signer", kind: "session" as const, address: SESSION_ACCOUNT.address, signer: SESSION_ACCOUNT as Address | typeof SESSION_ACCOUNT, zeroPrompt: true },
];
...
describe.each(VARIANTS)("$name", (v) => {
  ...
  describe("capabilities", () => {
    it("says which signer it is and whether a trade needs a prompt", () => {
      const { trade } = setup();
      expect(trade.capabilities).toMatchObject({ kind: variant.kind, address: variant.address, chainId: CHAIN, isZeroPrompt: variant.zeroPrompt });
    });
  });
