...
#   packages/deployments/sepolia.json   the deployed addresses (copy the contracts repo's deployments/sepolia.json here)
#   ../EVM-Pumpfun-Smart-Contract/.env  SEPOLIA_RPC_URL: your own RPC, used by the indexer, the API and the bot (never the browser)
#   apps/api/.env                       PINATA_JWT: token images and metadata are pinned to IPFS for real
#   apps/web/.env.local                 NEXT_PUBLIC_PRIVY_APP_ID and NEXT_PUBLIC_RPC_URL, as for `pnpm dev`
# What it creates, and keeps between runs (so comments, stars and profiles survive): the database vezta_demo, and apps/bot/.env
...
[ -s "$ROOT/packages/deployments/sepolia.json" ] || die "packages/deployments/sepolia.json is missing: copy the contracts repo's deployments/sepolia.json there"
...
# The RPC is read from the file, never echoed. Only the indexer, API and bot get it: the browser has its own (apps/web/.env.local).
RPC=""
