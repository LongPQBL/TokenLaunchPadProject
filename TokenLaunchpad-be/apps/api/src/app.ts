...
export function createApp(deps: Partial<AppDeps> = {}): Hono<AppEnv> {
  ...
  app.route("/", meRoutes({ pinner: deps.pinner, ipfsGateway: deps.ipfsGateway ?? DEFAULT_IPFS_GATEWAY }));
  ...
  chain.route("/addresses", profileRoutes({ ipfsGateway: deps.ipfsGateway ?? DEFAULT_IPFS_GATEWAY }));
